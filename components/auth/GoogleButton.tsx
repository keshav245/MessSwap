...
export default function GoogleButton({ onClick, loading = false, label = 'Continue with Google' }: GoogleButtonProps) {
  return (
    <button
      onClick={onClick}
      disabled={loading}
      className="w-full flex items-center justify-center gap-3 glass rounded-md py-3 text-sm font-medium hover:border-white/30 hover:bg-white/5 transition-all disabled:opacity-50"
    >
